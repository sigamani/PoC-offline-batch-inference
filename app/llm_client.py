...
class LLMClient:
    ...
    def generate_batch(self, prompts: List[str], max_tokens: int = 100, temperature: float = 0.7) -> Dict:
        """Generate batch text"""
        payload = {
            "prompts": prompts,
            "max_tokens": max_tokens,
...
def main():
    ...
    if args.health:
        ...
        print(f"   GPU Memory: {health.get('gpu_memory_used_gb', 'Unknown')}GB / {health.get('gpu_memory_total_gb', 'Unknown')}GB")
        print(f"   Queue Size: {health.get('queue_size', 'Unknown')}")
        return
    
    # Single prompt
    if args.prompt:
        print(f"🚀 Processing single prompt...")
    ...
    print(f"   Average per prompt: {result['total_time']/len(result['results']):.2f}s")
    ...
    for i, res in enumerate(result['results']):
        ...
        print(f"   {i+1:2d}. Tokens: {res['tokens_generated']:3d} | Time: {res['inference_time']:5.2f}s | {res['text'][:60]}...")
    
    print(f"\n📈 Summary:")
    print(f"   Total Tokens Generated: {total_tokens}")
    print(f"   Total Inference Time: {total_inference_time:.2f}s")
    print(f"   Average Tokens/Second: {total_tokens/total_inference_time:.1f}")
    print(f"   Throughput: {len(result['results'])/result['total_time']:.1f} prompts/second")
