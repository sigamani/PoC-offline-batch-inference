...
inference_requests = Counter(
    "llm_api_requests_total", "Total API requests", ["endpoint", "method", "status"]
)
...
active_connections = Gauge("llm_api_active_connections", "Active API connections")

queue_size = Gauge("llm_api_queue_size", "Current request queue size")

kv_cache_utilization = Gauge(
    "llm_kv_cache_utilization_percent", "KV cache utilization percentage"
)
...
async def generate_text(request: InferenceRequest):
    ...
    try:
        ...
        inference_requests.labels(
            endpoint="/generate", method="POST", status="success"
        ).inc()
        inference_duration.labels(
            worker_id=worker.worker_id, model_name=worker.model_name
        ...
        inference_requests.labels(
            endpoint="/generate", method="POST", status="error"
        ).inc()
...
async def generate_batch(request: BatchInferenceRequest):
    ...
    try:
        ...
        # Record metrics
        inference_requests.labels(
            endpoint="/generate_batch", method="POST", status="success"
        ).inc()
        ...
        return BatchInferenceResponse(
            results=results, total_time=total_time, worker_id=worker.worker_id
        )

    except Exception as e:
        inference_requests.labels(
            endpoint="/generate_batch", method="POST", status="error"
        ).inc()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        active_connections.dec()
