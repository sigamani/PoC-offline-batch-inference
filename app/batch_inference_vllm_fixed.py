...
sla_compliance = Gauge(
    "batch_sla_compliance_hours", "SLA compliance - estimated completion time in hours"
)

throughput_requests_per_sec = Gauge(
    "batch_throughput_requests_per_sec", "Current throughput in requests per second"
)

progress_percentage = Gauge("batch_progress_percentage", "Batch completion percentage")

active_batches = Gauge("batch_active_count", "Number of active batch jobs")
...
class SLAMonitor:
    ...
    def check_sla(self):
        ...
        if not self.metrics.is_sla_compliant():
            ...
            elapsed_hours = (
                (time.time() - self.metrics.start_time) / 3600
    ...
    def update_prometheus_metrics(self):
        ...
        if self.metrics.is_sla_compliant():
            batch_requests_total.labels(
                status="compliant", dataset=self.metrics.dataset_name
            ).inc()
        else:
            batch_requests_total.labels(
                status="at_risk", dataset=self.metrics.dataset_name
            ).inc()
...
class BatchInferenceServer:
        ...
        try:
            ...
            if data_config["input_path"].startswith("s3://"):
                ...
                ds = ray.data.from_items(
                    [{"prompt": prompt} for prompt in sample_prompts]
                )
            else:
    ...
    def setup_vllm_processor(self):
        ...
        try:
            ...
            vllm_config = vLLMEngineProcessorConfig(
                ...
                engine_kwargs={
                    "max_num_batched_tokens": inference_config.get(
                        "max_num_batched_tokens", 16384
                    ),
                    "max_model_len": model_config.get("max_model_len", 32768),
                    "gpu_memory_utilization": inference_config.get(
    ...
    def run_batch_inference(self):
        ...
        try:
            ...
            logger.info(
                f"Starting batch inference: {self.metrics.total_requests} requests"
            )
            ...
            logger.info(
                f"Final throughput: {self.metrics.throughput_per_sec():.2f} req/s"
            )
            logger.info(f"Total tokens: {self.metrics.tokens_processed}")

            # Record final metrics
            batch_duration.labels(
                model_name=self.metrics.model_name, dataset=self.metrics.dataset_name
            ).observe(total_time)

            batch_requests_total.labels(
                status="completed", dataset=self.metrics.dataset_name
