...
active_requests = Gauge("ray_serve_active_requests", "Active Ray Serve requests")
...
class VLLMInference:
    def __init__(self):
        ...
        self.worker_id = os.environ.get(
            "WORKER_ID", f"worker-{ray.get_runtime_context().get_node_id()}"
        )
    ...
    async def generate_batch(
        ...
        try:
            ...
            inference_requests.labels(
                deployment=self.deployment, method="generate_batch", status="success"
            ...
            inference_requests.labels(
                deployment=self.deployment, method="generate_batch", status="error"
            ).inc()
