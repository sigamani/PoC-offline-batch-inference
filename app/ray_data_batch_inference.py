...
class VLLMActor:
            ...
            while True:
                if torch.cuda.is_available():
                    for i in range(torch.cuda.device_count()):
                        ...
                        gpu_utilization.labels(node_id=self.node_id, gpu_id=str(i)).set(
                            utilization
                        )
                time.sleep(5)
    ...
    def infer_batch(
        ...
        inference_duration.labels(
            node_id=self.node_id, model_name=self.model_name
        ).observe(inference_time)

        # Add node info to results
        for result in results:
            result["node_id"] = self.node_id
            result["inference_time"] = inference_time / len(
                prompts
...
def initialize_ray_cluster():
    ...
    for i, node in enumerate(gpu_nodes):
        actor = VLLMActor.options(
            num_gpus=1, resources={f"node:{node['NodeID']}": 1}
        ).remote(model_name)
...
async def health():
    ...
    try:
        nodes = ray.nodes()
        gpu_nodes = [
            node for node in nodes if node.get("Resources", {}).get("GPU", 0) > 0
        ]
...
async def generate_batch(request: BatchInferenceRequest):
    ...
    try:
        ...
        # Record metrics
        inference_requests.labels(method="generate_batch", status="success").inc(
            len(request.prompts)
        )
        ...
        inference_requests.labels(method="generate_batch", status="error").inc()
