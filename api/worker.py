...
class BatchWorker:
    def __init__(
        ...
        gpu_scheduler: MockGPUScheduler | None = None,
    ):
        self.queue = queue
        config = BatchConfig()
        self.batch_dir = config.batch_dir if batch_dir is None else batch_dir
        self.pipeline = RayBatchProcessor(
            ModelConfig.default(), EnvironmentConfig.from_env()
        )
        self.running = False
        self.worker_thread = None
        self.gpu_scheduler = (
            gpu_scheduler if gpu_scheduler is not None else MockGPUScheduler()
        )
    ...
    def _process_job(
        self, job_data: Dict[str, Any], priority: priorityLevels = priorityLevels.LOW
    ) -> None:
        ...
        if not job_id or not input_file or not output_file:
            raise ValueError(
                f"Missing required job data: job_id={job_id}, input_file={input_file}, output_file={output_file}"
            )

        try:
            allocation_result = self.gpu_scheduler.allocate_gpu(
                job_id, priority_level=priority
            )
            if not allocation_result.allocated:
                logger.error(
                    f"Failed to allocate GPU for job {job_id}: {allocation_result.reason}"
                )
            ...
            if (
                hasattr(self.pipeline.env_config, "is_dev")
                and self.pipeline.env_config.is_dev
            ):
                ...
                try:
                    ...
                    results = self.pipeline._fallback_process(prompts)
                    logger.info(
                        f"Fallback inference completed, got {len(results)} results"
                    )
            ...
            if next_job_id:
                logger.info(
                    f"GPU freed from completed job {job_id}, allocated to waiting job {next_job_id}"
                )
            ...
            if next_job_id:
                logger.info(
                    f"GPU freed from failed job {job_id}, allocated to waiting job {next_job_id}"
                )
    ...
    def _update_job_status(
        self, job_id: str, status: str, completed_at: float | None = None
    ):
