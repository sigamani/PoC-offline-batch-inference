...
if __name__ == "__main__":
    ...
    def send_http_request(prompt, idx):
        ...
        try:
            response = requests.post(
                url, json={"text": prompt, "request_id": idx}, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Request {idx} completed in {elapsed:.2f}s by worker {result.get('worker_id')}"
                )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(send_http_request, prompt, i)
    ...
    if valid_results:
        ...
        workers = [
            r[0].get("worker_id") for r in valid_results if r[0].get("worker_id")
        ]
