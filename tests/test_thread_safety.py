...
class TestThreadSafety:
            ...
            for i in range(5):
                try:
                    msg_id = queue.enqueue(
                        {"job_id": f"prod_job_{i}"}, priorityLevels.LOW
                    )
    ...
    def test_scheduler_concurrent_allocation(self, scheduler):
        ...
        total_available = (
            final_status["spot"]["available"] + final_status["dedicated"]["available"]
        )
        total_capacity = (
            final_status["spot"]["capacity"] + final_status["dedicated"]["capacity"]
        )
